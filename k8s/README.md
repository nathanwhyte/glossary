...
- **`garage/`** - Garage distributed S3-compatible object storage manifests (StatefulSet, Services, StorageClass)
...
- **Garage**: Distributed S3-compatible object storage (3 replicas) with Longhorn storage
...
Garage provides distributed S3-compatible object storage with replication handled at the application level. This deployment uses Longhorn for underlying storage with single replica (Garage handles replication/quorum).

### Prerequisites

- Nodes tagged in Longhorn with the `garage` tag
- Nodes have `ssd` disks configured in Longhorn
- Longhorn storage system installed and `longhorn-nvme` StorageClass available

### 1. Configure Longhorn Nodes

Ensure your storage nodes are properly configured in Longhorn:

1. **Tag nodes with "garage"**: In Longhorn UI, tag the nodes that should host Garage volumes with the `garage` tag
2. **Configure SSD disks**: Ensure nodes have `ssd` disks configured in Longhorn

The StorageClass will automatically use these tags to place volumes on the correct nodes.

### 2. Create Garage Secrets

Create the RPC secret and admin key:
...
kubectl create secret generic garage-secrets \
  ...
  --from-literal=GARAGE_ADMIN_KEY="$ADMIN_KEY" \
  -n glossary
```
...
Create the dedicated StorageClass for Garage volumes:

```bash
kubectl apply -f garage/storageclass.yaml
```

### 4. Add Garage Helm Repository

Add the official Garage Helm repository:
...
### 5. Deploy Garage with Helm

First, check the chart's default values to understand the available options:

```bash
helm show values garage/garage
```

Deploy Garage using the Helm chart with our custom values:

```bash
helm install garage garage/garage -f k8s/garage/values.yaml -n glossary
```

**Note**: The `values.yaml` file is a starting point. You may need to adjust it based on the actual chart structure. The chart documentation is available at: https://git.deuxfleurs.fr/Deuxfleurs/garage/src/branch/main-v1/script/helm

The values file includes configuration for:
- Longhorn StorageClass (`garage-longhorn`) with garage tag and ssd disk selectors
- Resource limits
- Environment variables for Garage configuration

### 6. Wait for Pods to be Ready

```bash
# Wait for all 3 Garage pods to be ready
kubectl wait --for=condition=ready pod -l app=garage -n glossary --timeout=300s

# Check pod status
kubectl get pods -l app=garage -n glossary
```

### 7. (Optional) Bootstrap Cluster Layout

If you want to explicitly configure the cluster layout, you can create a bootstrap job or use the Garage CLI directly:

```bash
# Get admin key from secret
ADMIN_KEY=$(kubectl get secret garage-secrets -n glossary -o jsonpath='{.data.GARAGE_ADMIN_KEY}' | base64 -d)

# Set admin key
export GARAGE_ADMIN_KEY="$ADMIN_KEY"

# Configure layout (example - adjust as needed)
kubectl exec -it garage-0 -n glossary -- garage layout assign -z dc1 -c 1 garage-0.garage.glossary.svc.cluster.local:3901
kubectl exec -it garage-0 -n glossary -- garage layout assign -z dc1 -c 1 garage-1.garage.glossary.svc.cluster.local:3901
kubectl exec -it garage-0 -n glossary -- garage layout assign -z dc1 -c 1 garage-2.garage.glossary.svc.cluster.local:3901

# Apply layout
kubectl exec -it garage-0 -n glossary -- garage layout apply
```

**Note**: This step is optional. Garage can auto-discover and form a cluster, but explicit layout configuration provides better control over data placement.
...
# Check services
kubectl get svc -l app=garage -n glossary

# Check PVCs
kubectl get pvc -l app=garage -n glossary
...
**Storage**:
- Data volume: 50Gi per pod (configured in `values.yaml`)
- StorageClass: `garage-longhorn` (single replica)
- StorageClass uses Longhorn `garage` node tag and `ssd` disk selector
- StorageClass must be created separately (see step 3)

**Replication**:
- Garage handles replication/quorum (replication factor: 3)
- Longhorn volumes use single replica to avoid "double replication"
...
Garage exposes Prometheus metrics on port 3902 via the `garage-metrics` service.

Access metrics directly:
```bash
kubectl port-forward svc/garage-metrics 3902:3902 -n glossary
curl http://localhost:3902/metrics
```

Or access from within the cluster:
```bash
curl http://garage-metrics:3902/metrics
```
...
```bash
# Check pod status
kubectl describe pod garage-0 -n glossary

# Check events
kubectl get events --sort-by='.lastTimestamp' -n glossary

# Verify Longhorn nodes are tagged correctly
# Check in Longhorn UI that nodes have "garage" tag and "ssd" disks
```

**PVCs Not Binding**:
```bash
# Check StorageClass
kubectl get storageclass garage-longhorn
...
```bash
# Check pod logs
kubectl logs garage-0 -n glossary
kubectl logs garage-1 -n glossary
kubectl logs garage-2 -n glossary

# Verify RPC secret is set
kubectl get secret garage-secrets -n glossary

# Test peer connectivity
kubectl exec garage-0 -n glossary -- ping -c 1 garage-1.garage
```

**Bootstrap Job Failing**:
```bash
# Check job logs
kubectl logs job/garage-bootstrap -n glossary

# Verify pods are ready
kubectl get pods -l app=garage -n glossary

# Check if admin key is set
kubectl get secret garage-secrets -n glossary -o jsonpath='{.data.GARAGE_ADMIN_KEY}'
```

### Maintenance

**Updating Configuration**:
Edit `k8s/garage/values.yaml` and upgrade the Helm release:

```bash
helm upgrade garage garage/garage -f k8s/garage/values.yaml -n glossary
```
...
**Note**: Scaling up requires layout reconfiguration. Scaling down may affect quorum.

**Upgrading Garage Version**:
Update the image tag in `values.yaml`:

```yaml
image:
  tag: v1.3.0  # or latest
```
...
**Rolling Restart**:
```bash
kubectl rollout restart statefulset/garage -n glossary
```

**Uninstalling**:
```bash
helm uninstall garage -n glossary
```

**Note**: This will delete the Garage deployment but not the PVCs (data is preserved). To fully remove, delete PVCs manually if needed.

### Future S3 Integration

When S3 integration is needed, the following will be required:

- Create buckets via `garage bucket create`
- Generate S3 access keys via `garage key new`
- Create Secret with S3 credentials for applications
- Update application code to use S3 client
- Configure lifecycle policies if needed

For now, Garage is deployed and ready for future S3 integration.
...
- **Garage**:
  - RPC secret and admin key should be created via `kubectl` rather than committed to version control
  - Use `garage/secret.yaml.template` as a template only (contains placeholder values)
